...
public class UserAggDriver {
	...
	public static void main(String[] args) {
		...
		SparkSession sparkSession = SparkSession.builder()
				.config(new SparkConf())// .set("spark.executor.memory","5g").
									    // .setMaster("yarn").set("spark.driver.memory","4g"))
				.getOrCreate();
				

		Dataset<UserActivities> inuptDS = sparkSession
												.read()
												.option("header","true")
		...
		Dataset userAndDeviceAgg = inuptDS
				.groupBy(inuptDS.col("swid") , inuptDS.col("device_category"))
				.agg(sum(inuptDS.col("time_spent")).as("time_spent"), sum(inuptDS.col("video_start")).as("video_start"));
		
		userAndDeviceAgg.printSchema();
		
		Dataset	udfOutput = userAndDeviceAgg.groupBy(userAndDeviceAgg.col("swid"))
				.agg(callUDF("customAgg", userAndDeviceAgg.col("device_category"),
										  userAndDeviceAgg.col("time_spent"),
										  userAndDeviceAgg.col("video_start")
							).as("multicolumn")
					);
		...
		// generating new columns by splitting a column returned by UDF(multicolumn)
		Dataset output = udfOutput
		.withColumn("device_distribution", col("multicolumn.device_distribution"))
